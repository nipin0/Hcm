...
async def _read_config_fields(
    ...
    """Read config fields from hcm_config.metadata and return typed dict.
    ...
    try:
        ...
        rows = await db_pool.fetch(
            f"""SELECT config_key, current_value
                FROM hcm_config.metadata
                WHERE config_key IN ({placeholders})""",
            *keys,
        )
        for row in rows:
            full_key: str = row["config_key"]
            for i, k in enumerate(keys):
                if full_key == k:
                    field = field_names[i]
                    result[field] = _coerce_type(row["current_value"], defaults.get(field, ""))
                    break
    except Exception as exc:
        logger.warning("Config read from DB failed, using defaults: %s", exc)
...
def create_system_router(
    ...
    async def _get_deepseek_impl() -> dict:
        """Handler: read DeepSeek configuration with api_key masked."""
        config = await _read_config_fields(
            db_pool, DEEPSEEK_CONFIG_KEYS, DEEPSEEK_FIELD_NAMES, DEEPSEEK_FIELD_DEFAULTS,
        )
    ...
    async def _get_notifications_impl() -> dict:
        """Handler: read notification configuration."""
        config = await _read_config_fields(
            db_pool,
