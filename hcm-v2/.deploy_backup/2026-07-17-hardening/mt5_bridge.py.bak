...
DEFAULT_MAX_SIGNAL_AGE_SECONDS = 10
...
def place_mt5_order(mt5, signal_data, symbol, timeframe_str, redis_conn=None):
    ...
    if redis_conn:
        ...
        if ai_risk_on:
            ...
            if (ai_sl > 0 or ai_tp > 0) and atr > 0:
                ...
                if ai_tp > 0 and tp == 0:
                    tp_dist = atr * ai_tp
                    tp = (tick.ask + tp_dist) if direction == "BUY" else (tick.bid - tp_dist)
                log.info("P1c AI risk applied: ai_sl_mult=%.2f ai_tp_mult=%.2f → sl=%.2f tp=%.2f",
                         ai_sl, ai_tp, sl, tp)
    ...
    if redis_conn and tp == 0:
        try:
            ...
            if zone_tp > 0 and atr > 0:
                ...
                trail_start = _get_close_config(
                    redis_conn, "close.trail_start_atr_mult",
    ...
    log.info(
        f"Placing {direction} {lot} lot {symbol} @ {price} (bid={tick.bid} ask={tick.ask}) "
        ...
        if result['code'] == 0:
            ...
            async with pool.acquire() as conn:
                ...
                await conn.execute("""
                    ...
                    (account_id, symbol, direction, open_price, current_price, lot, sl, tp, open_time, signal_id)
...
async def _recheck_zone_pending(pool, mt5, redis_conn, dry_run: bool) -> None:
    ...
    for key in keys:
        ...
        if _price_in_zone_band(tick, direction, zl):
            ...
            redis_conn.delete(key)
            redis_conn.set(f"bridge:processed:{sid}", "done")
        # Not in band → keep waiting; Redis TTL will drop expired keys.
...
async def main(dry_run=False):
    ...
    try:
        consumers = redis_conn.xinfo_consumers("signal:risk_passed", "bridge-order-group")
        for c in consumers:
            if c.get("name") != cur_consumer:
                try:
                    redis_conn.xgroup_delconsumer(
                        "signal:risk_passed", "bridge-order-group", c["name"]
                    )
    ...
    try:
        ...
        redis_conn.xgroup_setid("signal:risk_passed", "bridge-order-group", "$")
        log.info(
        ...
        try:
            ...
            if now - last_risk > 2:
                try:
                    ...
                    msgs = None
                # 过期阈值（可热改）：bridge.max_signal_age_seconds，缺省 10s
                _ma = redis_conn.hget("hcm:config:v2", "bridge.max_signal_age_seconds")
                try:
                    max_signal_age = int(_ma) if _ma else DEFAULT_MAX_SIGNAL_AGE_SECONDS
                except (ValueError, TypeError):
                    max_signal_age = DEFAULT_MAX_SIGNAL_AGE_SECONDS
                if msgs:
                    ...
                    for _stream_name, entries in msgs:
                        ...
                        for dmid, dmsg in entries_sorted[1:]:
                            ...
                            redis_conn.set(f"bridge:processed:{dsid}", "done")
                            redis_conn.xack("signal:risk_passed", "bridge-order-group", dmid)
                        ...
                        if latest_prefix <= last_prefix:
                            ...
                            redis_conn.set(f"bridge:processed:{lsid}", "done")
                            redis_conn.xack("signal:risk_passed", "bridge-order-group", latest_id)
                            continue
                        ...
                        for msg_id, msg_data in [(latest_id, latest_data)]:
                            ...
                            if zone_trigger_on:
                                ...
                                if zt_wait > 0 and zl > 0:
                                    tick = _mt5_global.symbol_info_tick(msg_symbol)
...
def _update_trailing_stops(mt5, redis_conn):
    ...
    try:
        ...
        # 所有阈值均从 Redis/PG 读取（禁用散落硬编码；缺失键回退到 CLOSE_CONFIG_DEFAULTS 集中默认值）
        be_trigger = atr * _get_close_config(
            redis_conn, "close.breakeven_atr_mult",
            CLOSE_CONFIG_DEFAULTS["close.breakeven_atr_mult"],
        )
        be_buffer = atr * _get_close_config(
            redis_conn, "close.breakeven_buffer_atr_mult",
            CLOSE_CONFIG_DEFAULTS["close.breakeven_buffer_atr_mult"],
        )
        trail_wide = atr * _get_close_config(
            redis_conn, "close.trail_wide_atr_mult",
            CLOSE_CONFIG_DEFAULTS["close.trail_wide_atr_mult"],
        )
        # ── P0-2: 仅盈利超过该 ATR 倍数后才启动移动止盈线 ──
        # 之前只靠保本地板保护，让盈利单充分奔跑，避免被 trail_wide 贴身扫掉。
        trail_start = atr * _get_close_config(
            redis_conn, "close.trail_start_atr_mult",
            CLOSE_CONFIG_DEFAULTS["close.trail_start_atr_mult"],
        )
