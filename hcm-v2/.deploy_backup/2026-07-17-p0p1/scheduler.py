...
class Scheduler:
    ...
    async def _produce_signal(
        ...
        if not score_result.threshold_passed:
            ...
            if engine_reason:
                ...
                suppress_chain.append((10, f"below_threshold({score_result.pre_score:.3f}<{score_result.threshold:.3f})"))
        ...
        if cooldown == 0:
            if state.same_dir_bypass_count >= 3:
                ...
                # P2c: observable suppression.
                suppress_chain.append((20, "same_dir_bypass_limit(reached 3)"))
                await self._publish_filtered_signal(
                    state, indicators, regime_result, score_result, trace_id,
                    zone_level=zone_level, zone_type=zone_type, zone_strength=zone_strength,
                    suppress_reason=_format_suppress_reason(suppress_chain),
        ...
        # ── Step 8: Final Direction (P1b: symmetric AI veto) ──
        # A high-confidence indicator score must NOT be overridden by a
        # low-confidence AI — whether the AI says NO_TRADE OR a REVERSE
        # direction. Only when the AI's confidence >= the score does the AI
        # win. This closes defect 2 (one-way veto asymmetry).
        # AI 门控优化(2026-07-16): AI 返回 NO_TRADE 默认视为"弃权"，不否决
        # 已通过 floor+threshold 的评分信号 —— 防止 AI 单方面杀死可交易单。
        # 仅当 scoring.ai_allow_notrade_veto=true 时，AI 才可用 NO_TRADE 否决。
        if ai_response is not None and ai_response.success:
            ai_dir = ai_response.direction
            ai_conf = ai_response.confidence
            score = score_result.pre_score
            if ai_dir == "NO_TRADE":
                # AI 弃权：保留评分方向(除非显式允许 AI 以 NO_TRADE 否决)
                if self._ai_allow_notrade_veto:
                    final_direction = "NO_TRADE"
                    final_confidence = ai_conf
                    logger.info(
                        "AI NO_TRADE veto applied (ai_allow_notrade_veto=true, conf=%.2f) → %s",
                        ai_conf, state.symbol,
                    )
                    suppress_chain.append((30, f"ai_notrade_veto(conf={ai_conf:.2f})"))
                else:
                    final_direction = score_result.direction
                    final_confidence = score
                    logger.info(
                        "AI NO_TRADE treated as ABSTAIN (ai_allow_notrade_veto=false, "
                        "conf=%.2f) → keeping scoring direction=%s (%s)",
                        ai_conf, final_direction, state.symbol,
                    )
                    suppress_chain.append((30, f"ai_notrade_abstain(conf={ai_conf:.2f})"))
            elif ai_dir != score_result.direction and score >= 0.20 and ai_conf < score:
                final_direction = score_result.direction
                final_confidence = score
                logger.info(
                    "AI veto overridden by strong scoring: ai_dir=%s ai_conf=%.2f "
                    "beats score=%.2f → %s",
                    ai_dir, ai_conf, score, final_direction,
                )
                suppress_chain.append((30, f"ai_veto_overridden(ai_dir={ai_dir},ai_conf={ai_conf:.2f}<score={score:.2f})"))
            else:
        ...
        if final_direction == "NO_TRADE":
            ...
            # P2c: attribute the NO_TRADE (AI veto takes priority 30).
            # B3 fix: engine's canonical reason (e.g. adx_floor) takes priority
            # over generic "no_trade_direction" so the dashboard shows the
            # *real* reason the signal was blocked.
            if ai_response is not None and ai_response.success and ai_response.direction == "NO_TRADE":
                suppress_chain.append((30, "ai_no_trade"))
            engine_reason = score_result.fallback_reason
            if engine_reason:
        ...
        ai_ok = ai_response is not None and ai_response.success
        ai_sl_mult = ai_response.sl_atr_multiplier if ai_ok else 0.0
        ai_tp_mult = ai_response.tp_atr_multiplier if ai_ok else 0.0
        suggested_lot = ai_response.suggested_lot_ratio if ai_ok else 1.0
        # If a usable structure zone exists and aligns with direction, let the
        # bridge wait for price to TOUCH the zone before filling (P1a). A
        # finite timeout (no dead order). 0 = fill immediately at market.
        if zone_level > 0 and (
            (final_direction == "BUY" and zone_type in ("SUPPORT", "PIVOT"))
            or (final_direction == "SELL" and zone_type in ("RESISTANCE", "PIVOT"))
        ):
            entry_trigger_wait = 900  # 15 min (M5 3 bars) timeout
        else:
            entry_trigger_wait = 0

        # ── P0/Zone: TP 锚点 = 对向结构 zone（下一阻力/支撑）──
        # 对向结构 zone 始终传给桥做 TP 锚点；上方 zone 与方向对齐时已无条件
        # 设置 entry_trigger_wait=900（延迟入场提示字段）。桥端是否据此等价格
        # 触 zone 才成交，由 signal_tower.zone_trigger_enabled（当前=true，见配置）
        # 控制 —— 本处只产出提示字段，不在此处开关延迟逻辑。
        zone_tp_level = (
        ...
        signal_data = SignalData(
            ...
            signal_mode=(
                ...
                else "ai_dynamic" if ai_response is not None
    ...
    async def load_config(self) -> None:
        ...
        try:
            ...
            # Hot-reload per-symbol regime/scoring thresholds
            await self._load_symbol_thresholds()
            # Hot-reload live override + AI 门控 参数(落库键：scoring.live_override_* / scoring.ai_allow_notrade_veto)
            await self._load_gate_config()
            logger.info(
