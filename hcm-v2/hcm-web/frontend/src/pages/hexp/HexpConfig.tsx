...
const FIELDS: Record<string, ConfigField[]> = {
  ...
  extreme: [
    ...
    { key: 'hexp.momentum_flip_trend_enabled', label: '趋势单保护', type: 'switch', defaultValue: true,
      description: '开启后：仅"顺势趋势单"（主周期 TREND_UP/DOWN 且方向同向，或 ADX≥阈值且 ma 方向一致）在正常回撤/换手中 M1 动量短暂反向不被误杀，改用更宽的反向阈值；逆势单仍敏捷拦截。修复"趋势单被动量否决误拦"。', suggested: '开启' },
    { key: 'hexp.momentum_flip_trend_adx', label: '趋势单 ADX 阈值', type: 'number', defaultValue: 25.0, min: 15, max: 40, step: 1,
      description: '辅助判据：主周期 ADX≥此值视为强趋势，触发趋势单保护（放宽反向阈值）。ADX>25 为标准强趋势线。', suggested: '25' },
    { key: 'hexp.momentum_flip_trend_mm', label: '趋势单反向阈值', type: 'number', defaultValue: 0.15, min: 0.05, max: 0.5, step: 0.01,
      description: '趋势单下放宽后的反向阈值：|mm|≥此值(剧烈反向)才拦趋势单。越大越宽松（趋势单越不易被误杀），但也放过更剧烈的反转。', suggested: '0.15' },
    ...
    // ── 趋势启动候选（2026-08-31 判定重构：波动率压缩→突破 + H1 共振）──
    { key: 'hexp.trend_start_observe_enabled', label: '趋势启动观测开关', type: 'switch', defaultValue: true,
      description: '开启后产出 trend_start_candidate 落库，供影子评估用未来 K 线模拟成交。零实盘影响。', suggested: '开启' },
    { key: 'hexp.trend_start_mode', label: '趋势启动判定模式', type: 'select', defaultValue: 'squeeze_breakout', suggested: 'squeeze_breakout',
      options: [
        { label: 'squeeze_breakout（新·压缩突破+H1共振，回测胜率 62~67%）', value: 'squeeze_breakout' },
        { label: 'phase_ignite（旧·相位点火，实测胜率 4%，仅回退用）', value: 'phase_ignite' },
      ],
      description: '新判定＝波动率压缩(bbw<20)→Donchian(10)突破→H1 EMA50 共振，回测样本内外净 R 同正（内 64 次胜率 61.8%，外 29 次胜率 66.7%）。旧判定实测胜率仅 4%、单均 -0.804R，仅作热回退保留。' },
    { key: 'hexp.trend_start_order_enabled', label: '趋势启动真下单', type: 'switch', defaultValue: false,
      description: '开启后趋势启动候选可绕过 grade 闸门真下单。默认关闭：新判定尚处影子验证期，且代码层对新判定另有禁单保护（双保险）。', suggested: '关闭' },
    { key: 'hexp.trend_start.lot_tier', label: '趋势启动手数分档', type: 'select', defaultValue: 'low', suggested: 'low',
      options: [
        { label: 'low（最小档 → 风控 lot_multiplier_low=0.5）', value: 'low' },
        { label: 'mid（中档 → 风控 lot_multiplier_mid=1.0）', value: 'mid' },
        { label: 'high（高档 → 风控 lot_multiplier_high=1.5）', value: 'high' },
      ],
      description: '趋势启动单的手数档位，实际倍率由风控引擎 risk.lot_multiplier_* 决定（链动动态手数，signal_tower 侧只选档、不乘固定系数）。默认 low 最小档。' },
    { key: 'hexp.trend_start_order_lot_mult', label: '趋势启动手数微调系数', type: 'number', defaultValue: 1.0, min: 0.05, max: 2, step: 0.05,
      description: '在动态手数基础上的微调系数，默认 1.0 = 完全不干预（手数交给 lot_tier + 风控决定）。仅在确需额外缩放时才配置，不应作为主要仓位控制手段。', suggested: '1.0' },
    { key: 'hexp.trend_start.new_mode_order_allowed', label: '新判定开放下单', type: 'switch', defaultValue: false,
      description: '新判定(squeeze_breakout)真下单的独立开关，与「趋势启动真下单」为「与」关系，两者同开才生效。默认关闭。影子验证达标（建议 ≥30 条、胜率 ≥45%、单均净 R>0.05）后再开；出问题关掉此项即可秒级回退到纯观测。', suggested: '关闭' },
    { key: 'hexp.trend_start.bbw_max', label: '压缩分位上限', type: 'number', defaultValue: 20.0, min: 5, max: 50, step: 1,
      description: '波动率压缩判据：bbw 带宽处于近 120 根的此分位以下才算压缩。越小越严（信号更少更精）。回测 20 显著优于 30/40。', suggested: '20' },
    { key: 'hexp.trend_start.don_look', label: '突破 Donchian 周期', type: 'number', defaultValue: 10, min: 5, max: 40, step: 1,
      description: '收盘突破近 N 根高低点（滞后一根防前视）。回测 10 优于 20/40。', suggested: '10' },
    { key: 'hexp.trend_start.h1_ema', label: 'H1 共振 EMA 周期', type: 'number', defaultValue: 50, min: 10, max: 200, step: 5,
      description: '多周期共振：最近已完成 H1 棒收盘 > H1 EMA(此周期) 只做多，反之只做空。这是唯一经样本内外双验证有效的过滤器（胜率 +12pp、单均 R 翻倍）。', suggested: '50' },
    { key: 'hexp.trend_start.sl_atr_mult', label: '趋势启动 SL ATR 倍数', type: 'number', defaultValue: 3.5, min: 1.0, max: 8, step: 0.1,
      description: '影子评估用 SL 距离（回测最优 3.5，40 组参数中 30 组样本内外同正）。与 exec.sl_atr_mult=2.0 口径不同，勿混淆。', suggested: '3.5' },
    { key: 'hexp.trend_start.rr', label: '趋势启动盈亏比', type: 'number', defaultValue: 1.5, min: 0.5, max: 4, step: 0.1,
      description: 'TP = SL × 此值。回测 RR=1.5 明显优于 2.0/3.0（RR 越大胜率衰减越快）。', suggested: '1.5' },
    { key: 'hexp.trend_start.hold_bars', label: '趋势启动持有根数', type: 'number', defaultValue: 90, min: 20, max: 240, step: 10,
      description: '持有上限（M5 根数，90 根=7.5 小时）。回测 90 与 150 等效、60/120 略降，属不敏感参数。', suggested: '90' },
  ],
  ...
  exec: [
    ...
    { key: 'hexp.exec.transition_lot_mult', label: '转换态降仓系数', type: 'number', defaultValue: 0.5, min: 0.1, max: 1, step: 0.05,
      description: '触发 TRANSITION（犹豫带）减仓时仓位再乘此值。配合"仅主周期"开关使用。', suggested: '0.5' },
    { key: 'hexp.exec.transition_primary_only', label: '转换态降仓仅看主周期', type: 'switch', defaultValue: true,
      description: '开启后：仅主执行周期(M5)处于 TRANSITION 犹豫带才降仓；辅助周期(如D1)犹豫不影响。修复"主周期明确趋势、仅辅助周期犹豫"的顺势趋势单被误减半仓。关闭则回退旧行为(任一周期犹豫即降仓)。', suggested: '开启' },
